...
def test_weights_and_cash_sum_to_100():
    ...
    plan = optimize_allocation(decisions, regime="上昇", vix=15.0)
    assert pytest.approx(sum(plan.weights.values()) + plan.cash_pct, abs=1e-6) == 100.0
    # 上昇相場の現金下限10%は守られる
    assert plan.cash_pct >= config.CASH_FLOOR_BY_REGIME["上昇"] * 100 - 1e-6
...
def test_no_eligible_names_all_cash():
    ...
    plan = optimize_allocation(decisions, regime="横ばい", vix=18.0)
    assert plan.weights == {}
...
def test_risk_off_raises_cash_floor():
    ...
    plan = optimize_allocation(decisions, regime="上昇", vix=32.0)
